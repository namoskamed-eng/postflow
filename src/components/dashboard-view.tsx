...
import { AlertTriangle, Bell, CalendarCheck, CalendarClock, History, Play, Plus, Sunrise } from "lucide-react";
...
export function DashboardView({ posts, clients, recentCount, onOpen, onNew, onCalendar, onPublication, onRecent }: { posts: Post[]; clients: Client[]; recentCount: number; onOpen: (post: Post) => void; onNew: () => void; onCalendar: () => void; onPublication: () => void; onRecent: () => void }) {
  ...
  return <div><div className="mb-7 flex flex-wrap items-end justify-between gap-3"><div><p className="mb-1 text-sm font-semibold text-[#7A7A73]">Visão da sua rotina</p><h1 className="font-display text-3xl font-extrabold tracking-tight sm:text-4xl">Hoje</h1></div><div className="flex flex-wrap justify-end gap-2"><InstallAppButton compact />{permission === "default" && <Button variant="secondary" onClick={enableNotifications}><Bell size={17} /><span className="hidden sm:inline">Ativar alertas</span></Button>}<Button variant="secondary" onClick={onRecent}><History size={17} />Recentes{recentCount > 0 && <span className="rounded-full bg-[#D9FF57] px-2 py-0.5 text-xs text-black">{recentCount}</span>}</Button><Button variant="secondary" onClick={onPublication}><Play size={17} />Publicar</Button><Button onClick={onNew} disabled={!clients.length}><Plus size={18} />Novo post</Button></div></div><div className="mb-8 grid grid-cols-2 gap-3 lg:grid-cols-4">{cards.map(({ label, value, icon: Icon, color }) => <button onClick={onCalendar} key={label} className="rounded-2xl border border-[#E2E2DC] bg-white p-4 text-left"><div className={`mb-4 grid size-10 place-items-center rounded-xl ${color}`}><Icon size={19} /></div><div className="font-display text-3xl font-extrabold">{value}</div><div className="text-xs font-semibold text-[#77776F]">{label}</div></button>)}</div>{!posts.length ? <EmptyState icon={<CalendarCheck />} title="Tudo em ordem" text="Não há postagens ativas no momento." action={clients.length ? <Button onClick={onNew}><Plus size={18} />Criar postagem</Button> : undefined} /> : <><Section title="Atrasados" items={groups.late} /><Section title="Para hoje" items={groups.today} /><Section title="Amanhã" items={groups.tomorrow} /><Section title="Próximos 7 dias" items={groups.week} /></>}</div>;
}
