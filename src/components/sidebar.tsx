...
export function Sidebar({ view, setView, onLogout }: { view: View; setView: (view: View) => void; onLogout: () => void }) {
  return <>
    ...
    <nav aria-label="Navegação principal" className="fixed inset-x-3 bottom-3 z-40 flex items-center justify-around rounded-2xl border border-white/80 bg-[#1D1D1B]/95 p-2 shadow-xl backdrop-blur lg:hidden">{items.map(({ id, label, icon: Icon }) => <button key={id} aria-label={label} title={label} onClick={() => setView(id)} className={cn("grid size-10 shrink-0 place-items-center rounded-xl transition", view === id ? "bg-[#D9FF57] text-black shadow-sm" : "text-white/55 hover:bg-white/10 hover:text-white")}><Icon size={20} strokeWidth={view === id ? 2.5 : 2} /></button>)}<button aria-label="Sair" title="Sair" onClick={onLogout} className="grid size-10 shrink-0 place-items-center rounded-xl text-white/55 transition hover:bg-white/10 hover:text-white"><LogOut size={20} /></button></nav>
  </>;
}
